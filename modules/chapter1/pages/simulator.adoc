...
**Task:** Provision a Red Hat OpenShift cluster with the OpenShift AI 3.0 operator installed; selectable with 1-4 accelerators.
++++
...
**Task:** Log in to the RHOAI console, review the dashboard, inspect the "Hardware Profiles" to understand allocate physical GPUs for inference, then deploy a model from the RHOAI Model Catalog.
++++
...
**Task:** From the Openshift Dashboard install the "Leader Worker Set (LWS)" Operator. This is the secret sauce `llm-d` uses to manage pod fleets.  Explore OpenShift console to validate a version of 4.19 or greater, with 4.20 preferred is installed.  Install the Terminal operator to interact with our cluster via the command line.
++++
