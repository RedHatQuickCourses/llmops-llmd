 ...
 ** **Continuous Batching:**  Batching requests is to maximize GPU usage by processing multiple queries together rather than sequentially.
...
The combined software stack ensures these expensive resources are utilized efficiently:  

 * *vLLM ensures the GPUs are not idle during execution cycles,* 
 * *llm-d ensures that requests are routed to the right GPUs at the right time.*