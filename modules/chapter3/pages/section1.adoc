...
The framework is deeply integrated with cloud-native principles and Kubernetes, consisting of three main layers:

1.  **Intelligent Routing Layer:** Built on the Kubernetes Gateway API and its new Inference Extension, this layer makes dynamic, telemetry-driven scheduling decisions.
2.  **Distributed Model Serving Layer:** Utilizes the high-performance **vLLM engine** for core inference operations.
3.  **Sophisticated, Pluggable KV Cache Hierarchy:** Manages the critical state ("short-term memory") of inference requests across the cluster.

=== Performance Gains
The framework’s core technical advantages translate into powerful empirical gains:

 *   **3x LOWER Average Time-To-First-Token (TTFT):** Users perceive your application as being significantly faster, addressing complaints that the chatbot is "slow to start".
 *   **2x HIGHER Queries-Per-Second (QPS):** Allows you to serve double the users with the same hardware.
 *   **Massive TCO Reduction:** Stop buying more GPUs and start using the ones you have more efficiently, ensuring AI projects are economically viable.

These gains are driven by features like **Prefill/Decode (P/D) disaggregation** and **KV-cache-aware routing**, amplified by strategic collaboration with NVIDIA’s Dynamo ecosystem for accelerated data transfer and resource planning.
...
2.  **Path #2: Prefill/Decode (P/D) Disaggregation (Microservices for AI)**
    *   This path splits the inference process, preventing one "clunky server" from juggling both compute-heavy Prefill and memory-heavy Decode.
