...
| **The "Brain"** of `llm-d`. It is a plug-in to the Gateway that uses the Endpoint Picker Protocol (EPP). It only makes decisions—it does not move data. It maintains a real-time view of all vLLM pods and runs its "Filter-and-Score" logic to pick the best destination.
...
| **Dependant Operators**
| **Supporting Infrastructure**. The Cert Manager Operator, Arthorino Operator, and the **Leader-Worker\_set Operator** must be installed to deploy resources using distributed inference with `llm-d`.
|===
